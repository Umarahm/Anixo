...
import { Send, UserCircle, LogIn, AlertCircle, Reply, X, MessageCircle, Users, Shield, Crown } from 'lucide-react';
        ...
        <div className="flex-grow flex flex-col bg-[#121212]/60 backdrop-blur-2xl md:border border-white/5 rounded-none md:rounded-3xl overflow-hidden shadow-none md:shadow-[0_0_40px_rgba(0,0,0,0.5)] relative">
          ...
          <div className="bg-[#1a1a1a]/60 backdrop-blur-md border-b border-white/10 p-3 px-4 md:p-5 md:px-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 z-10 relative">
            <div className="flex flex-col pr-2">
              ...
              <p className="text-white/40 text-[10px] md:text-[13px] mt-1 leading-snug">Real-time public chat room. Messages disappear after 24 hours.</p>
            </div>
            ...
            <div ref={chatContainerRef} className="flex-grow overflow-y-auto p-3 md:p-8 flex flex-col gap-4 md:gap-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              {messages.length === 0 ? (
                ...
                messages.map((msg, index) => {
                  ...
                  return (
                    <div key={msg._id || index} className={`group flex gap-3 max-w-[90%] md:max-w-[75%] ${isMe ? 'ml-auto flex-row-reverse' : ''}`}>
                      {/* Avatar */}
                      ...
                      <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                        <div className="flex items-baseline gap-2 mb-1.5 px-2">
                          <Link to={`/user/${msg.profileId || msg.username}`} className={`text-[13px] font-semibold transition-colors flex items-center gap-1.5 ${msg.role === 'admin' ? 'text-purple-500 hover:text-purple-400' : msg.role === 'moderator' ? 'text-red-500 hover:text-red-400' : 'text-white/70 hover:text-red-400'}`}>
                        ...
                        {msg.replyTo && (
                          <div className={`mb-2 px-2.5 py-1.5 rounded-md text-[11px] border-l-[3px] leading-snug ${
                            isMe 
                    ...
                    {user && (
                      <button 
                        ...
                        className={`opacity-0 group-hover:opacity-100 transition-all duration-200 text-white/30 hover:text-white hover:bg-white/10 p-2 rounded-full self-end mb-1 ${isMe ? 'mr-1' : 'ml-1'}`}
                        title="Reply"
                      >
                        <Reply size={16} />
                      </button>
